...
# load your DSU data
with open('path_to_your_DSU_train.pkl', 'rb') as f1:
    train_audio = pickle.load(f1)
with open(path + 'path_to_your_DSU_val.pkl', 'rb') as f2:
    val_audio = pickle.load(f2)
        
train_audio = np.array(train_audio)
val_audio = np.array(val_audio)
train_audio = torch.from_numpy(train_audio)
...
class Text2TextModel(nn.Module):
    ...
    def forward(self, source_ids, source_mask, target_ids, target_mask, audio): # all inputs' shape: bsz * max_len
...
for epoch in range(num_epochs):
    ...
    for i in range(0, len(train_data), batch_size):
        ...
        batch_audio = train_audio[i:i+batch_size].to(DEVICE)
...
# evaluation
    ...
    for i in range(0, len(val_data), batch_size):
        ...
        batch_audio = val_audio[i:i+batch_size].to(DEVICE)
